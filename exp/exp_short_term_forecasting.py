...
class Exp_Short_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        # 为非M4数据集添加默认frequency_map
        if not hasattr(self.args, 'frequency_map') or self.args.frequency_map is None:
            if self.args.data == 'm4':
                # M4数据集会在_build_model中设置frequency_map
                pass
            else:
                # 为exchange_rate数据集设置与M4日度数据相同的frequency_map
                self.args.frequency_map = 1  # 与M4 Daily相同
